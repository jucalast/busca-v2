        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-zinc-600 text-sm">O especialista está criando seu plano...</p>
                    <p className="text-zinc-500 text-xs mt-2">Pesquisando dados reais e gerando ações</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-white">
            <div className="max-w-3xl mx-auto px-6 py-8">
                ...
                <div className="flex items-start gap-3 mb-6">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-zinc-900">{planData.titulo_plano || pillarLabel}</h1>
                        <p className="text-zinc-600 text-sm mt-1">{planData.objetivo}</p>
                ...
                {/* Progress */}
                {planStatus === 'approved' && (
                    <div className="mb-6 p-4 rounded-xl bg-zinc-50">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-zinc-600">Progresso de execução</span>
                            <span className="text-xs font-mono text-zinc-700">{completedCount}/{totalActions}</span>
                        </div>
                    ...
                    <div className="mb-6 p-5 rounded-2xl border-2 border-dashed border-amber-500/20 bg-amber-500/[0.03]">
                        ...
                        <div className="flex gap-2 mt-3">
                            <input
                                ...
                                className="flex-1 bg-zinc-100 rounded-xl px-3 py-2 text-sm text-zinc-700 placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-300 transition-all"
                            />
                            <button
                ...
                <section className="mb-6">
                    <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-[0.2em] mb-3">
                        Plano de ação
                    </h2>
                            ...
                            return (
                                <div key={action.id}
                                    ...
                                    <div className="flex items-start gap-3 p-4">
                                        ...
                                        {planStatus === 'approved' ? (
                                            ...
                                            >
                                                {isDone
                                                    ...
                                                    : <Circle className="w-5 h-5 text-zinc-400 hover:text-zinc-600 transition-colors" />
                                                }
                                            </button>
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <p className={`text-sm font-medium ${isDone ? 'text-zinc-500 line-through' : 'text-zinc-900'}`}>
                                                    {action.titulo}
                                                </p>
                                                <PriorityBadge priority={action.prioridade} />
                                            </div>
                                            {action.descricao && (
                                                <p className="text-zinc-600 text-xs leading-relaxed mb-2">{action.descricao}</p>
                                            )}
                                            ...
                                            {action.resultado_esperado && !isDone && (
                                                <p className="text-[11px] text-zinc-600/80 mt-2 pl-0 border-l-2 border-emerald-200 leading-relaxed italic">
                                    ...
                                    {isTracking && !isDone && planStatus === 'approved' && (
                                        <div className="px-4 pb-4 border-t border-zinc-200 pt-3 ml-8">
                                            <p className="text-[11px] text-zinc-600 mb-2">O que aconteceu ao executar esta ação?</p>
                                            <input
                                                type="text"
                                                value={outcomeText}
                                                onChange={(e) => setOutcomeText(e.target.value)}
                                                placeholder="Ex: Criei o perfil no Google Meu Negócio"
                                                className="w-full bg-zinc-100 rounded-lg px-3 py-2 text-xs text-zinc-700 placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-300 mb-2"
                                            />
                                            <input
                                                type="text"
                                                value={impactText}
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 rounded-lg text-xs text-zinc-500 hover:text-zinc-700 transition-colors"
                                                >
                                                    Cancelar
                                                </button>
                                            </div>
                ...
                {(planData.kpis_pilar?.length ?? 0) > 0 && (
                    <section className="mb-6">
                        <button
                            ...
                            className="flex items-center gap-2 text-xs font-semibold text-zinc-500 uppercase tracking-[0.2em] mb-3 hover:text-zinc-700 transition-colors"
                        >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {planData.kpis_pilar.map((kpi, i) => (
                                    <div key={i} className="p-3 rounded-xl bg-zinc-50">
                                        <p className="text-zinc-900 text-sm font-medium">{kpi.nome}</p>
                                        <div className="flex items-center justify-between mt-1.5">
                                            <span className="text-[11px] text-zinc-600">Atual: {kpi.valor_atual}</span>
                                            <span className="text-[11px] text-emerald-700">Meta: {kpi.meta}</span>
                                        </div>
                                        {kpi.como_medir && (
                                            <p className="text-[10px] text-zinc-500 mt-1">{kpi.como_medir}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                ...
                {planData.resultado_final && (
                    <div className="mb-6 p-4 rounded-xl bg-emerald-50">
                        <p className="text-[10px] font-semibold text-emerald-700 uppercase tracking-wide mb-1">Resultado final esperado</p>
                        <p className="text-sm text-zinc-800 leading-relaxed">{planData.resultado_final}</p>
                    </div>
                )}

                {/* Connection to next pillars */}
                {planData.conexao_proximos_pilares && (
                    <div className="mb-6 p-4 rounded-xl bg-violet-50">
                        <p className="text-[10px] font-semibold text-violet-700 uppercase tracking-wide mb-1">Conexão com outros pilares</p>
                        <p className="text-sm text-zinc-800 leading-relaxed">{planData.conexao_proximos_pilares}</p>
                    </div>
                )}
                ...
                {(planData.sources?.length ?? 0) > 0 && (
                    <section className="mb-6">
                        <h2 className="flex items-center gap-2 text-xs font-semibold text-zinc-500 uppercase tracking-[0.2em] mb-3">
                            ...
                            {(planData.sources ?? []).map((url, i) => {
                                ...
                                return (
                                    <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                                        className="flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-lg bg-zinc-100 text-zinc-600 hover:text-zinc-800 transition-colors">
