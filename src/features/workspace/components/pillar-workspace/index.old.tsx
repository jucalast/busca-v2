...
import { safeRender, openInGoogleDocs, openInGoogleSheets, openInGoogleForms, exportAsCSV, exportFullAnalysis, getToolInfo } from './utils';
...
export default function PillarWorkspace({
    ...
    if (selectedPillar) {
        ...
        // Compute documents produced by subtasks for this pillar
        ...
        return (
            <div className="h-screen bg-[#09090b] flex">
                ...
                <div className="flex-1 min-w-0 flex flex-col pt-0 relative z-30 overflow-hidden" style={{ backgroundColor: 'lab(5 0 0)' }}>
                    {/* Full-width progress bar at the very top of the task column */}
                    {totalTasks > 0 && (
                    ...
                    {/* Top Top Bar: Task Progress & Tab Switch */}
                    {totalTasks > 0 && (
                        <div className="flex flex-col px-6 pt-5 pb-3 gap-2">
                            <div className="flex items-center justify-between">
                                ...
                                {/* Tab switcher: Tarefas / Documentos */}
                                {docsForDropdown.length > 0 && (
                    ...
                    <div className="flex-1 px-3 pb-28 overflow-visible flex flex-col relative">
                        ...
                        {activeRightTab === 'docs' && (
                            <div className="flex-1 flex flex-col overflow-hidden">
                                {(() => {
                                    // Group docs by task
                                    const docsByTask: Array<{ tid: string; taskTitle: string; taskIndex: number; docs: typeof docsForDropdown }> = [];
                                    ...
                                    for (const doc of docsForDropdown) {
                                        ...
                                        if (!(doc.tid in taskMap)) {
                                            taskMap[doc.tid] = docsByTask.length;
                                            docsByTask.push({ tid: doc.tid, taskTitle, taskIndex, docs: [] });
                                        }
                                        docsByTask[taskMap[doc.tid]].docs.push(doc);
                                    }

                                    const toggleFolder = (tid: string) => {
                                        setOpenFolders(prev => {
                                            const next = new Set(prev);
                                    ...
                                    const handleDocOpen = (doc: typeof docsForDropdown[0], fmt: string) => {
                                    ...
                                    const totalDocs = docsForDropdown.length;

                                    return (
                                        <div className="flex flex-col h-full">
                                            {/* Index / Summary bar */}
                                            <div className="shrink-0 mx-1.5 mb-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                                                <div className="flex items-center justify-between mb-2.5">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div>
                                                            <span className="text-[12px] font-semibold text-zinc-300">Documentos</span>
                                                            <span className="text-[10px] text-zinc-600 ml-2">{totalDocs} arquivo{totalDocs !== 1 ? 's' : ''}</span>
                                                        </div>
                                                    </div>
                                                    <button
                                                        onClick={() => {
                                                            const allOpen = docsByTask.every(g => openFolders.has(g.tid));
                                                            if (allOpen) {
                                                                setOpenFolders(new Set());
                                                            } else {
                                                                setOpenFolders(new Set(docsByTask.map(g => g.tid)));
                                                            }
                                                        }}
                                                        className="text-[9px] text-zinc-600 hover:text-zinc-400 transition-colors cursor-pointer px-2 py-1 rounded-md hover:bg-white/[0.04]"
                                                    >
                                                        {docsByTask.every(g => openFolders.has(g.tid)) ? 'Recolher tudo' : 'Expandir tudo'}
                                                    </button>
                                                </div>
                                                {/* Quick index */}
                                                <div className="flex flex-wrap gap-1">
                                                    {docsByTask.map((group) => (
                                                        <button
                                                            key={group.tid}
                                                            onClick={() => {
                                                                setOpenFolders(prev => new Set(prev).add(group.tid));
                                                                document.getElementById(`folder-${group.tid}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                                                            }}
                                                            className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.04] hover:border-white/[0.08] transition-all duration-150 cursor-pointer group"
                                                        >
                                                            <Hash className="w-2.5 h-2.5 text-zinc-600 group-hover:text-zinc-400" />
                                                            <span className="text-[9px] font-medium text-zinc-500 group-hover:text-zinc-300 truncate max-w-[140px]">
                                                                Tarefa {group.taskIndex + 1}
                                                            </span>
                                                            <span className="text-[9px] text-zinc-700 font-mono">{group.docs.length}</span>
                                                        </button>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex-1 overflow-y-auto px-1.5 pb-4">
                                                {docsByTask.map((group) => {
                                                    const isOpen = openFolders.has(group.tid);
                                                    return (
                                                        <div key={group.tid} id={`folder-${group.tid}`} className="mb-1">
                                                            {/* Folder row */}
                                                            <button
                                                                onClick={() => toggleFolder(group.tid)}
                                                                className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg hover:bg-white/[0.03] transition-all duration-150 cursor-pointer group"
                                                            >
                                                                <ChevronRight className={`w-3 h-3 text-zinc-600 transition-transform duration-200 ${isOpen ? 'rotate-90' : ''}`} />
                                                                {isOpen
                                                                    ? <FolderOpen className="w-4 h-4 text-amber-400/80" />
                                                                    : <Folder className="w-4 h-4 text-amber-400/60 group-hover:text-amber-400/80" />
                                                                }
                                                                <span className="flex-1 text-[12px] font-medium text-zinc-400 group-hover:text-zinc-200 text-left truncate transition-colors">
                                                                    {group.taskTitle}
                                                                </span>
                                                                <span className="text-[9px] font-mono text-zinc-700 bg-white/[0.03] px-1.5 py-0.5 rounded">
                                                                    {group.docs.length}
                                                                </span>
                                                            </button>
                                                            ...
                                                            {/* Files inside folder */}
                                                            {isOpen && (
                                                                <div className="ml-5 pl-3 border-l border-white/[0.04]">
                                                                    {group.docs.map((doc, docIdx) => {
                                                                        ...
                                                                        return (
                                                                            <div
                                                                                ...
                                                                                className="group/file relative mb-1.5 rounded-xl border border-white/[0.03] bg-white/[0.015] hover:bg-white/[0.04] hover:border-white/[0.07] transition-all duration-200 overflow-hidden cursor-pointer"
                                                                                onClick={() => handleDocOpen(doc, primaryFmt)}
                                                                            >
                                                                                {/* Document preview header */}
                                                                                <div className="px-3.5 pt-3.5 pb-2">
                                                                                    {/* Fake document lines for preview effect */}
                                                                                    <div className="mb-3 space-y-1.5 opacity-40 group-hover/file:opacity-60 transition-opacity">
                                                                                        <div className="h-[3px] rounded-full bg-zinc-600 w-[75%]" />
                                                                                        <div className="h-[3px] rounded-full bg-zinc-700 w-[90%]" />
                                                                                        <div className="h-[3px] rounded-full bg-zinc-700 w-[60%]" />
                                                                                        <div className="h-[3px] rounded-full bg-zinc-700 w-[80%]" />
                                                                                    </div>

                                                                                    {/* Title */}
                                                                                    <p className="text-[11px] font-medium text-zinc-300 leading-snug line-clamp-2 group-hover/file:text-white transition-colors">
                                                                                        {doc.title}
                                                                                    </p>
                                                                                </div>

                                                                                {/* Footer with format icons */}
                                                                                <div className="flex items-center justify-between px-3.5 py-2 border-t border-white/[0.03] bg-white/[0.01]">
                                                                                    <div className="flex items-center gap-1.5">
                                                                                        {formats.map((fmt: string) => {
                                                                                            const isLoadingFmt = loadingDoc === `${doc.tid}_${doc.idx}_${fmt}`;
                                                                                            return (
                                                                                                <button
                                                                                                    key={fmt}
                                                                                                    disabled={isLoadingFmt}
                                                                                                    title={fmtLabel(fmt)}
                                                                                                    onClick={(e) => {
                                                                                                        e.stopPropagation();
                                                                                                        handleDocOpen(doc, fmt);
                                                                                                    }}
                                                                                                    className="flex items-center gap-1 px-1.5 py-0.5 rounded-md hover:bg-white/[0.06] transition-colors disabled:opacity-50 cursor-pointer"
                                                                                                >
                                                                                                    {isLoadingFmt ? (
                                                                                                        <Loader2 className="w-3.5 h-3.5 animate-spin text-zinc-500" />
                                                                                                    ) : (
                                                                                                        <img src={fmtIcon(fmt)} alt={fmtLabel(fmt)} className="w-4 h-4 object-contain" />
                                                                                                    )}
                                                                                                    <span className="text-[9px] text-zinc-600 group-hover/file:text-zinc-500">{fmtLabel(fmt)}</span>
                                                                                                </button>
                                                                                            );
                                                                                        })}
                                                                                    </div>
                                                                                    <ExternalLink className="w-3 h-3 text-zinc-700 group-hover/file:text-zinc-500 transition-colors" />
                                                                                </div>
                                                                            </div>
                                                                        );
                                                                    })}
